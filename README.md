...
| `AWG_CPU_C2S` | Нет | CPU для потока client→server (-1 = авто, по умолчанию: -1) |
| `AWG_CPU_S2C` | Нет | CPU для потока server→client (-1 = авто, по умолчанию: -1) |
| `AWG_BUSY_POLL` | Нет | SO_BUSY_POLL таймаут в мкс (0 = выкл, по умолчанию: 0) |
