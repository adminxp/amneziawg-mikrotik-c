...
Версия протокола определяется автоматически: **v3** если задан `AWG_HEADER_PROTECTION_KEY`, иначе **v2** если заданы S3/S4 или H в виде диапазонов, иначе **v1.5** если заданы CPS-шаблоны (I1-I5), иначе **v1**.
...
IPv6 поддерживается **только на плече «контейнер → AWG-сервер»** (`AWG_REMOTE`). Приём от WireGuard-клиента (`AWG_LISTEN`) остаётся IPv4: это veth внутри самого роутера, где второй стек ничего не добавляет, а таблица сессий и кэш профилей хранят адрес клиента как `sockaddr_in`.
...
Обратите внимание: IPv6 поддерживается **только на плече до сервера**. `AWG_LISTEN` (приём от WireGuard-клиента роутера) остаётся IPv4 -- это локальный veth внутри роутера, где IPv6 ничего не даёт.
