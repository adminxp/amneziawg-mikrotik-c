...
| `AWG_SRC_PORT` | Нет | auto | Исходящий порт к серверу: `auto` / число / `random` |
...
**`AWG_SRC_PORT`** -- исходящий UDP-порт для соединения с AWG-сервером. По умолчанию (`auto`) прокси использует порт клиента WireGuard -- это нужно для корректной работы NAT на роутере. Если задано число, используется фиксированный порт. Значение `random` отдаёт выбор порта ядру: без bind, новый эфемерный порт при каждом реконнекте -- полезно за CGN/NAT провайдера, где переиспользование старой пары порт+сервер после обрыва может залипать.

**Конфигуратор генерирует `random`**, и вот почему. Залипнуть может не только NAT провайдера, но и masquerade на самом роутере. Если запись conntrack создалась в момент, когда у WAN-интерфейса не было адреса (типично сразу после переполучения аренды DHCP), RouterOS подставляет в неё адрес какого-нибудь другого своего интерфейса -- скажем, приватный адрес из туннельной сети. Провайдер такие пакеты выбрасывает, ответов нет. При фиксированном порте эта запись **не истекает никогда**: UDP-таймаут conntrack 30 секунд, а прокси шлёт handshake каждые 5, поэтому запись обновляется быстрее, чем стареет. Хуже того, реконнект прокси не спасает -- он пересоздаёт сокет с той же 5-tuple и попадает в ту же испорченную запись. Канал остаётся мёртвым, пока запись не удалят руками, при полностью живом сервере на другом конце. С `random` каждый реконнект берёт новый эфемерный порт, а значит и новую запись conntrack -- NAT пересчитывается сам, и канал поднимается за секунды.
...
AWG_SRC_PORT=random  # эфемерный порт ядра, новый при каждом реконнекте
                     # (это значение ставит конфигуратор)
```
